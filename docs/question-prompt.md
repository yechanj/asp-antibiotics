...
- **보기 길이·구체성 편향 금지 (중요).** "정답만 길고 상세하게 적고, 오답은 단답으로 짧게" 쓰지 마세요. 답을 몰라도 **가장 길고 자세한 보기를 고르면 맞는** 문제가 됩니다. 규칙:
  - 네 보기의 **길이·문장 구조·구체성 수준을 비슷하게** 맞춘다(예: 모두 한 구절/모두 한 문장).
  - 정답에만 붙이는 한정어·부연설명("~하지만 ~때문에", "~를 포함하여")을 **오답에도 같은 밀도로** 부여한다.
  - 오답도 정답만큼 구체적인 오개념(그럴듯하지만 틀린 기전·spectrum·판단)으로 작성한다. 단순히 "MRSA를 커버한다" 같은 짧은 단답으로 처리하지 않는다.
  - 상세한 부연은 보기가 아니라 `explanation`/`choice_explanations`에 넣는다.
