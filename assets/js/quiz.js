/* =========================================================
   데일리 테스트 (ASP 문제은행 기반 복습)
   - assets/data/questions/index.json → 각 강의 JSON 로드
   - 강의 섞어 랜덤 10문제 / 오답 다시 풀기
   - 점수·정답·오답을 localStorage에 저장
   순수 JS · 외부 의존성 없음
   ========================================================= */
(function () {
  "use strict";
  ...
  var DAILY_COUNT = 10;
  var LS_STATS = "asp_quiz_stats";     // { id: {seen, correct, wrong} }
  var LS_HISTORY = "asp_quiz_history"; // [{date, mode, score, total}]
  var LS_CYCLE = "asp_quiz_cycle";     // 이번 회차에 이미 출제된 문제 id 배열
  ...
  function levelKo(lv) { return lv === 1 ? "핵심" : lv === 2 ? "기전" : "응용"; }
  ...
  function startSession(mode) {
    ...
    current = pool.map(function (q) {
      ...
      // 보기별 해설도 같은 순서로 재정렬(정렬 어긋남 방지)
      var ce = q.choice_explanations || [];
      var expl = order.map(function (i) { return ce[i] || ""; });
      var newAnswer = order.indexOf(q.answer);
      return { q: q, opts: opts, expl: expl, answer: newAnswer, picked: null };
    });
    pos = 0;
    sessionScore = 0;
    renderQuestion();
  ...
  function renderQuestion() {
    ...
    el.stage.innerHTML =
      "<div class='qd-quiz'>" +
        ...
        "<div class='qd-meta'>" +
          "<span class='qd-chip qd-chip--dis'>" + escapeHtml(q.lesson || "") + "</span>" +
          "<span class='qd-chip qd-chip--con'>" + escapeHtml(q.concept || "") + "</span>" +
          (q.source_type === "extended" ? "<span class='qd-chip qd-chip--ext'>교재 밖 확장</span>" : "") +
        "</div>" +
  ...
  function pick(i) {
    ...
    var picks = "";
    if (!ok && item.expl[i]) {
      picks += "<div class='qd-ce qd-ce--no'><b>내가 고른 " + String.fromCharCode(65 + i) + ".</b> " + escapeHtml(item.expl[i]) + "</div>";
    }
    if (item.expl[item.answer]) {
      picks += "<div class='qd-ce qd-ce--ok'><b>정답 " + String.fromCharCode(65 + item.answer) + ".</b> " + escapeHtml(item.expl[item.answer]) + "</div>";
    }
    ...
    exp.innerHTML = "<b class='qd-exp-h'>" + (ok ? "정답입니다" : "오답입니다") + "</b>" +
      "<span class='qd-exp-txt'>" + escapeHtml(item.q.explanation) + "</span>" + picks;
    exp.hidden = false;
    ...
    var wrongHtml = wrongs.length
      ? wrongs.map(function (it) {
          ...
          return "<div class='qd-rev'>" +
            ...
            "<a class='qd-rev-link' href='lessons/" + q.lessonRef + "'>→ " + escapeHtml(q.lesson || "강의") + "에서 복습</a>" +
          "</div>";
        }).join("")
